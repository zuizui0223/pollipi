...
def generate_balanced_trial_plan(
    *,
    ...
    recording_day: str,
    setup_id: str,
    target_truth_schedule_id: str,
    nuisance_truth_schedule_id: str,
    seed: int = DEFAULT_PLAN_SEED,
    n_development_per_cell: int = 12,
    n_heldout_per_cell: int = 24,
) -> list[dict[str, object]]:
    if not all(x.strip() for x in (experiment_id, recording_day, setup_id, target_truth_schedule_id, nuisance_truth_schedule_id)):
        raise ValueError("experiment/setup/day and both truth schedule IDs must be non-empty")
    if n_development_per_cell <= 0 or n_heldout_per_cell <= 0:
        raise ValueError("per-cell counts must be positive")

    rng = random.Random(int(seed))
    rows: list[dict[str, object]] = []
    for role, rounds in (("development", int(n_development_per_cell)), ("heldout", int(n_heldout_per_cell))):
...
def manifest_for_role(
    *, experiment_id: str, prospective_role: str, setup_id: str,
    primary_source_id: str, nuisance_reference_source_id: str,
    nuisance_truth_source_id: str, target_truth_source_id: str,
    frame_interval_s: float, plan_seed: int = DEFAULT_PLAN_SEED,
) -> dict[str, object]:
    return {
        "schema": SCHEMA,
...
def write_plan_bundle(
    *, output_dir: str | Path, experiment_id: str, recording_day: str, setup_id: str,
    target_truth_schedule_id: str, nuisance_truth_schedule_id: str,
    primary_source_id: str, nuisance_reference_source_id: str,
    nuisance_truth_source_id: str, target_truth_source_id: str,
    frame_interval_s: float, seed: int = DEFAULT_PLAN_SEED,
    n_development_per_cell: int = 12, n_heldout_per_cell: int = 24,
) -> dict[str, str]:
    out = Path(output_dir); out.mkdir(parents=True, exist_ok=True)
    rows = generate_balanced_trial_plan(
        experiment_id=experiment_id, recording_day=recording_day, setup_id=setup_id,
        target_truth_schedule_id=target_truth_schedule_id,
        nuisance_truth_schedule_id=nuisance_truth_schedule_id,
        seed=seed, n_development_per_cell=n_development_per_cell,
    ...
    for role in ("development", "heldout"):
        manifest = manifest_for_role(
            experiment_id=experiment_id, prospective_role=role, setup_id=setup_id,
            primary_source_id=primary_source_id,
            nuisance_reference_source_id=nuisance_reference_source_id,
            nuisance_truth_source_id=nuisance_truth_source_id,
            target_truth_source_id=target_truth_source_id,
            frame_interval_s=frame_interval_s, plan_seed=seed,
        )
        manifest["trial_plan_sha256"] = digest
        (out / f"{role}_manifest.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (out / "PLAN_SHA256.txt").write_text(f"{digest}  trial_plan.canonical.json\n", encoding="utf-8")
    return {"trial_plan_csv": str(csv_path), "development_manifest": str(out / "development_manifest.json"), "heldout_manifest": str(out / "heldout_manifest.json"), "sha256": digest}
