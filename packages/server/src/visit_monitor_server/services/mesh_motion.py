...
def analyze_mesh_motion(
    ...
    *,
    ...
    """Classify motion as no activity, environmental noise, or visitation candidate.

    This is a deterministic rectangular mesh baseline with a half-cell offset mesh.
    The public shape intentionally leaves room for a future hex layout without
    changing the capture loop contract.
    """
    import numpy as np
...
def compact_mesh_metrics(metrics: dict[str, Any]) -> dict[str, Any]:
    """Drop non-serializable internals before storing status/log metadata."""
